...
simple_distri_plot(registered, 'race_code', 'Race', 'Registered Voters by Race in Wayne County (Incomplete Data)')

# Distribution of registered voters over 100 by race in the registered voter dataset
simple_distri_plot(registered.loc[registered["birth_year"]<=1920], 'race_code', 'Race', 'Registered Voters Over 100 by Race in Wayne County (Incomplete Data)')

# Distribution of all dead registered voters by birth_year
simple_distri_plot(dead_registered, 'BirthYear', 'Birth Year', 'Dead Voters by Birth Year in Wayne County', False)
...
whole = pd.concat([dead_registered, registered.rename(columns=rename)], join='inner')
...
ziptorange(dead_registered, detroit['zip'], 'ZipCode', 'detroit')
checkthebalance(dead_registered, 'detroit', 'Voted', 0.05)
